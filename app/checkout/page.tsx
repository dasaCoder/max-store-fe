...
import {
  addDecimalPoints,
  formatCurrency,
} from "../lib/features/util/currency-formatter";
...
export default function CheckoutForm() {
  ...
  const { currencyCode } = useSelector((state: RootState) => state.app);
  ...
  useEffect(() => {
    const fetchHash = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/hash`);
  ...
  useEffect(() => {
    setPayhereHash(
      md5(
        `${merchantId}${orderId}${addDecimalPoints(
          subtotal
        )}${currencyCode}${md5(hash).toUpperCase()}`
      ).toUpperCase()
    );
  }, [subtotal, hash]);
          ...
          >
            <div className="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-12 xl:gap-16">
              <div className="min-w-0 flex-1 space-y-8">
                ...
                <DeliveryDetails
                  formObj={
                    paymentForm as unknown as FormikProps<DeliveryFormFields>
                  }
                />
              ...
              <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
                <OrderSummary subTotal={subtotal} currencyCode={currencyCode} deliveryFee={100} tax={0} savings={0} total={subtotal}/>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    {JSON.stringify(paymentForm.errors)}
                    <a
                      href="#"
                      title=""
